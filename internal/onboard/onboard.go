...
func (s *session) summary(res Result) {
	...
	s.say("  ./starlight-agent -config %s -validate-config   # check it", res.ConfigPath)
	s.say("  ./starlight-agent -config %s -task \"what to do\"", res.ConfigPath)
}
...
func renderCredentials(envKey, key string) string {
	return fmt.Sprintf(
		"# Generated by starlight-agent -init. This file holds a secret: keep it\n"+
