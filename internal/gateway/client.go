...
// SetLLM changes the provider and/or model on the gateway and refreshes the cache.
// An empty argument means "leave unchanged" on the server side.
func (c *Client) SetLLM(provider, model string) {
	_ = c.do(context.Background(), http.MethodPatch, c.scoped("/config"), map[string]string{
		"provider": provider,
		"model":    model,
	}, nil)
	c.mu.Lock()
...
func (c *Client) SetWorkspace(dir string) { _ = dir }

// GenerateTitle is a no-op on the remote client: the title is generated by
// the gateway's runner after the first turn. The method exists to satisfy
// the Service interface.
func (c *Client) GenerateTitle(_ context.Context, firstUserMessage string) string {
	return strings.Join(strings.Fields(firstUserMessage), " ")
}

// RestoreTranscript is a no-op on the remote client: the transcript is
// restored by the gateway when a persisted session is loaded. The method
// exists to satisfy the Service interface.
func (c *Client) RestoreTranscript(turns []agent.DialogueTurn) { _ = turns }
