...
.PHONY: help build 386 amd64 arm64 all test vet fmt fmt-check check clean run smoke-386
...
386: ## Compila el binario i386 (linux/386) — el objetivo principal
	GOOS=linux GOARCH=386 CGO_ENABLED=0 $(GO) build -trimpath -ldflags "$(LDFLAGS)" -o $(DIST)/starlight-linux-386 .
...
all: 386 amd64 arm64 ## Compila los tres binarios estáticos
...
smoke-386: 386 ## Prueba el binario i386 dentro de un contenedor de 32 bits
	docker run --rm --platform linux/386 -v $(CURDIR)/$(DIST):/t:ro i386/debian:bookworm-slim /t/starlight-linux-386 -version
